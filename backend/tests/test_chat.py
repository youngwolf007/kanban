...
class TestRejectingBadResponses:
    ...
    def test_a_transient_upstream_failure_is_retried(self, signed_in, monkeypatch, ai):
        from app.ai import AIError
    ...
    def test_a_board_that_is_not_an_object_is_rejected(self, signed_in, ai):
        """The schema says object or null, but a provider is free to ignore it."""
        before = signed_in.get("/api/board").json()
        ai.response = {"reply": "Done.", "board": "the whole board, honestly"}

        response = signed_in.post("/api/chat", json={"message": "Change it"})

        assert response.status_code == 502
        assert response.json()["detail"] == "The AI returned an invalid board"
        assert signed_in.get("/api/board").json() == before

    def test_a_board_that_is_an_array_is_rejected(self, signed_in, ai):
        before = signed_in.get("/api/board").json()
...
class TestBoundingTheRequest:
    def test_an_over_long_message_is_rejected(self, signed_in, ai):
        response = signed_in.post(
            "/api/chat", json={"message": "x" * (MAX_MESSAGE_LENGTH + 1)}
        )

        assert response.status_code == 422
        assert ai.calls == 0

    def test_a_message_at_the_limit_is_accepted(self, signed_in, ai):
        response = signed_in.post(
            "/api/chat", json={"message": "x" * MAX_MESSAGE_LENGTH}
        )

        assert response.status_code == 200

    def test_an_over_long_history_entry_is_rejected(self, signed_in, ai):
        response = signed_in.post(
            "/api/chat",
            json={
                "message": "hello",
